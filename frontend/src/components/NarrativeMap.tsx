...
export const NarrativeMap: React.FC<NarrativeMapProps> = ({ arguments: args, onBubbleClick }) => {
  const memoizedPositions = useMemo(() => {
    ...
    args.forEach(arg => {
      ...
      if (arg.cluster_id !== null && arg.cluster_id !== -1) {
        ...
        pos = new THREE.Vector3().copy(center).add(new THREE.Vector3(
          (Math.random() - 0.5) * 2,
          (Math.random() - 0.5) * 2,
          (Math.random() - 0.5) * 2
        ));
  ...
  return (
    <Canvas camera={{ position: [0, 0, 15], fov: 50 }}>
      ...
      {args.map((arg) => {
        const color = arg.cluster_id === null || arg.cluster_id === -1 
          ? '#576574'
          : clusterColors[arg.cluster_id % clusterColors.length];
